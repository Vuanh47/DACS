...
public class ThanhToan extends JFrame implements ActionListener{
	...
	public void bill() {
		  ...
		  int rowIndex = row(); // Giả sử row() trả về chỉ số hàng. Đảm bảo rằng nó không âm.
	        ...
	        StyledDocument doc = bill.getStyledDocument();

	        // Tạo một AttributeSet để định dạng văn bản
	        ...
	        StyleConstants.setAlignment(center, StyleConstants.ALIGN_CENTER);
	        StyleConstants.setFontSize(center, 15); 
	        
	        // Đặt định dạng cho văn bản
	        try {	
	        	
	        	doc.insertString(doc.getLength()," Trung tâm chăm sóc sức khỏe \n "
	        			+ "\t PHÚC AN\n", center);
	        
	            doc.insertString(doc.getLength(), "   Hóa đơn thanh toán viện phí\n", center);
	            doc.insertString(doc.getLength(), "----------------------------------------------------------\n", b);
	            doc.insertString(doc.getLength(), " \t\t    "+currentDate+"\n", b);
		           
	            doc.insertString(doc.getLength(), "Họ Tên:       " + row.elementAt(1)+" \n", c);
	            doc.insertString(doc.getLength(), "Mã Bệnh Nhân: "+ row.elementAt(0)+"\n", c);
	            doc.insertString(doc.getLength(), "Giới Tính:    " + row.elementAt(3)+" \n", c);
		           
	            doc.insertString(doc.getLength(), "Ngày Khám:     "+ row.elementAt(2)+" \n", c);
	            doc.insertString(doc.getLength(), "Số Phòng:   "+ row.elementAt(5)+" \n", c);
	            doc.insertString(doc.getLength(), "Bảo Hiểm Y Tế:  " +row.elementAt(7)+ " \n", c);
	            doc.insertString(doc.getLength(), "Chuẩn Đoán:     "+ row.elementAt(6)+" \n\n", c);
	            doc.insertString(doc.getLength(), "----------------------------------------------------------\n", b);
	            doc.insertString(doc.getLength(), "Viện Phí: "+row.elementAt(4), center);
